...
class HomeScreenState extends State<HomeScreen>
  ...
  void initState() {
    ...
    debugPrint('🏠 HOME SCREEN INSTANCE #$_instanceId CREATED');
    _loadUser();
  ...
  Future<void> refreshData() async {
    debugPrint('🔄 HOME SCREEN #$_instanceId: refreshData() called');
    if (!mounted) {
      debugPrint('⚠️ Home screen not mounted, skipping refresh');
      return;
    }
    await _refreshFromDb();
    debugPrint('🔄 HOME SCREEN #$_instanceId: refreshData() completed');
  }

  Future<void> _refreshFromDb() async {
    debugPrint('=== HOME SCREEN #$_instanceId: Starting _refreshFromDb ===');
    
    try {
      final rows = await _repo.getAll();
      debugPrint('Retrieved ${rows.length} transactions from database');
      ...
      debugPrint('Current: Year=${now.year}, Month=${now.month}');
      debugPrint('Processing ${rows.length} transactions...');

      for (final r in rows) {
        final dt = DateTime.tryParse(r['date'] as String) ?? now;
        // Only count transactions from current month (EXACTLY like Profile)
        if (dt.year == now.year && dt.month == now.month) {
          final amt = (r['amount'] as num).toDouble();
          final type = (r['type'] as String).toLowerCase().trim();
          
          debugPrint('  ✓ $type: ₹$amt (${r['category']})');
      ...
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      debugPrint('TOTALS: Income=₹$monthIncome, Expense=₹$monthExpense, Balance=₹$balance');
      debugPrint('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      ...
      debugPrint('Setting state with calculated values...');
      setState(() {
        _transactions
          ..clear()
          ..addAll(
      ...
      debugPrint('   _income = ₹$_income');
      debugPrint('   _expenses = ₹$_expenses');
      debugPrint('   _totalBalance = ₹$_totalBalance');
  ...
  Widget build(BuildContext context) {
    super.build(context);
    debugPrint('🎨 HOME SCREEN #$_instanceId BUILD called - Balance: ₹$_totalBalance, Income: ₹$_income, Expense: ₹$_expenses, Transactions: ${_transactions.length}');
    return Scaffold(
